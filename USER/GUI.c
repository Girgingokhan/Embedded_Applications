 ...
 * @parameters :sx:the start x coordinate of the specified area
                sy:the start y coordinate of the specified area
...
void LCD_Show_2412_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode)
{
    ...
    {		
			...
			{
				temp = (asc2_2412[num][pos*2]<<8)|asc2_2412[num][pos*2+1];
				for(t=0;t<12;t++)
				{                 
						if(temp&0x8000)
						{
...
void LCD_Show_3216_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode)
{
	...
	{
	    temp = (asc2_3216[num][pos*2]<<8)|asc2_3216[num][pos*2+1];
	    for(t=0;t<16;t++)
	    {                 
				if(temp&0x8000)
				{
...
void LCD_Show_4824_char(u16 x,u16 y,u16 fc, u16 bc, u8 num,u8 mode)
{
    ...
    u16 colortemp=POINT_COLOR;      		   
    num=num-' ';
    LCD_SetWindows(x,y,x+24-1,y+48-1);
    if(!mode)
    {		
	for(pos=0;pos<48;pos++)
	{
	    temp = (asc2_4824[num][pos*3]<<16)|(asc2_4824[num][pos*3+1]<<8)|asc2_4824[num][pos*3+2];
	    for(t=0;t<24;t++)
	    {                 
		if(temp&0x800000)
		{
...
{         
    ...
    {   
		...
		return;     
        LCD_Show_2412_char(x,y,POINT_COLOR,BACK_COLOR,*p,mode);
        x+=12;
        p++;
    }  
} 
...
{         
    ...
    {   
		if(x>(lcddev.width-1)||y>(lcddev.height-1)) 
		return;     
        LCD_Show_3216_char(x,y,POINT_COLOR,BACK_COLOR,*p,mode);
        x+=16;
        p++;
    }  
} 
...
{         
    ...
    {   
		if(x>(lcddev.width-1)||y>(lcddev.height-1)) 
		return;     
        LCD_Show_4824_char(x,y,BACK_COLOR,POINT_COLOR,*p,mode);
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_Show_2412_char(x+12*t,y,POINT_COLOR,BACK_COLOR,' ',0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_Show_2412_char(x+12*t,y,POINT_COLOR,BACK_COLOR,temp+'0',0); 
	}
} 
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_Show_3216_char(x+16*t,y,POINT_COLOR,BACK_COLOR,' ',0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_Show_3216_char(x+16*t,y,POINT_COLOR,BACK_COLOR,temp+'0',0); 
	}
} 
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_Show_4824_char(x+24*t,y,POINT_COLOR,BACK_COLOR,' ',0);
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_Show_4824_char(x+24*t,y,POINT_COLOR,BACK_COLOR,temp+'0',0); 
	}
} 
...
{					
    ...
    { 
        if(!bHz)
        {
					if(x>(lcddev.width-12)||y>(lcddev.height-24)) 
					return; 
						if(*str>0x80)bHz=1;
						else              
						{          
								...
								{
 									 LCD_Show_4824_char(x+24*t,y,fc,bc,*str,mode);
									 x+=24;
								}
						 } 
								str++; 
...
{					
    ...
    { 
        if(!bHz)
        {
					if(x>(lcddev.width-8)||y>(lcddev.height-16)) 
					return; 
						if(*str>0x80)bHz=1;
						else              
						{          
								...
								else
								{
 									 ...
 									 LCD_Show_3216_char(x+16*t,y,fc,bc,*str,mode);
									 x+=16;
								}
						 } 
								str++; 
...
{					
    ...
    { 
        if(!bHz)
        {
					if(x>(lcddev.width-6)||y>(lcddev.height-12)) 
					return; 
						if(*str>0x80)bHz=1;
						else              
						{          
								...
								else
								{
 									 ...
 									 LCD_Show_2412_char(x+12*t,y,fc,bc,*str,mode);
									 x+=12;
								}
						 } 
								str++; 
...
{
	u16 len=strlen((const char *)str);
	u16 x1=(lcddev.width*8)/2;
	Show_Str3216(x1,y,fc,bc,str,mode);
} 
/*****************************************************************************
